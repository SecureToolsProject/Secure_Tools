...
import {
  ...
  inspectMetadata,
  verifyMetadata,
} from "../../../assets/vendor/secure-metadata/secure-metadata-0.1.0.browser.js";
...
function assertSupportedReport(report, detectedFormat) {
  if (!report || !IMAGE_FORMATS[report.format] || report.format !== detectedFormat) {
    throw metadataError("IMAGE_METADATA_UNSUPPORTED");
  }
}
...
export async function inspectImageMetadata(file) {
  ...
  let detectedFormat;
  try {
    detectedFormat = await validateImageSignature(file);
  } catch (error) {
    throw metadataError(error?.code || "IMAGE_SIGNATURE_INVALID", error);
  }
  const bytes = new Uint8Array(await file.arrayBuffer());
  try {
    const report = inspectMetadata(bytes);
    assertSupportedReport(report, detectedFormat);
    return Object.freeze({
      file,
      bytes,
      format: detectedFormat,
      mimeType: IMAGE_FORMATS[detectedFormat].mimeType,
      report,
...
export function createCleanOutputPlan(source) {
  ...
  return {
    filename: createImageOutputNames([source.file], source.format, { suffix: "_clean" }).entries[0],
    format: source.format,
    mimeType: IMAGE_FORMATS[source.format].mimeType,
    extension: `.${IMAGE_FORMATS[source.format].extension}`,
  };
}

export async function cleanAndVerifyImageMetadata(source) {
  if (!source?.bytes || !source.cleanable) throw metadataError("IMAGE_METADATA_NOT_CLEANABLE");
  const plan = createCleanOutputPlan(source);
  let cleaned;
  try {
    cleaned = cleanMetadata(source.bytes, DEFAULT_CLEANING_POLICY);
  } catch (error) {
    throw mapLibraryError(error, "IMAGE_METADATA_CLEANING_FAILED");
  }
  let verification;
  try {
    verification = verifyMetadata(cleaned.output);
  } catch (error) {
    throw mapLibraryError(error, "IMAGE_METADATA_VERIFICATION_FAILED");
  }
  const checksPassed = Array.isArray(verification?.checks)
    && verification.checks.length > 0
    && verification.checks.every((check) => check?.passed === true);
  const reportComplete = verification?.report?.inspectionStatus !== "container-partial"
    && verification?.report?.metadataTruncated !== true;
  if (verification?.valid !== true || !checksPassed || !reportComplete || cleaned?.format !== source.format) {
    throw metadataError("IMAGE_METADATA_VERIFICATION_FAILED");
  }
  return {
    plan,
    blob: new Blob([cleaned.output], { type: plan.mimeType }),
    bytes: cleaned.output,
    cleaned,
    verification,
  };
}
